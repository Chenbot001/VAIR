...
class Motor:
    def __init__(self, id, portHandler, packetHandler) -> None:
        ...
        self.vel_operating_mode = 1 # 速度模式
        self.pos_operating_mode = 3 # 位置模式
        self.extpos_operating_mode = 4 # 扩展位置模式
        self.currpos_operating_mode = 5 # 推荐使用，位置力控模式
        self.pwm_operating_mode = 16 # PWM模式
    ...
    def pos_force_mode(self, goal_current=800, goal_pwm=500):
        """
        设置力控位置模式
        """
        self.torq_off()
    ...
    def init_config(self, curr_limit=800, goal_current=500, goal_pwm=500):
        """
        电机初始配置，LED闪烁，设置为力控位置模式，并使能电机
        """
        for i in range(2):
            self.led_on()
            time.sleep(0.1)
