...
def on_press(key):
    ...
    try:
        ...
        # --- Object Diameter Adjustment (J/K keys) ---
        elif key.char == 'j':  # Decrease diameter by 1mm
            state.object_diameter_mm = max(CONFIG["min_diameter_mm"], 
                                         state.object_diameter_mm - 1)
        elif key.char == 'k':  # Increase diameter by 1mm
            state.object_diameter_mm = min(CONFIG["max_diameter_mm"], 
                                         state.object_diameter_mm + 1)
